...
export async function DELETE(request: NextRequest) {
  try {
    ...
    // Security check: users can only delete avatars for themselves unless they're admin
    if (userId !== user.id) {
    ...
    const path = `Avatars/${userId}-${safeName}`;
    ...
    if (storageError) {
      ...
      return NextResponse.json(
        { error: storageError.message || 'Failed to delete avatar from storage' },
        { status: 500 }
      );
    }
    ...
    if (dbError) {
      console.error('Error updating database after avatar deletion:', dbError);
      return NextResponse.json(
    ...
    console.error('Unexpected error in delete avatar API:', error);
    return NextResponse.json(
